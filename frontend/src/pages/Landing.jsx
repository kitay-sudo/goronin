...
// Поля:
//   handle    — с @ или без, отрисуется единообразно
//   amount    — строка для отображения, например "0.66 TON" или "10 USDT"
//   amountTon — число в TON для сортировки (если донат не в TON, пересчитай к TON или просто
//               поставь любую сравнимую величину; используется ТОЛЬКО для определения «top»)
//   addedAt   — дата в ISO ("2026-04-28"), нужна чтобы выявлять «первого по времени» (firstEver)
//   note      — опционально, короткая ремарка
//
// Карточки на странице расставляются автоматически:
//   • TOP DONOR  — у кого amountTon максимален (золотой акцент 一)
//   • FIRST EVER — у кого addedAt самый ранний (серебряный акцент 初)
//   Если это один и тот же человек — он получает обе плашки в одной карточке.
const DONORS = [
  {
    handle: '@sounee',
    ...
    addedAt: '2026-04-28',
    note: 'первый воин, ступивший на путь поддержки',
  },
];
    ...
    <div className="mt-10">
      ...
      {!empty && (
        <p className="mb-5 text-xs text-zinc-500 leading-relaxed">
          ...
          <span className="text-zinc-300 font-mono">初 first ever</span> — кто пришёл самым первым.
          Внёс больше всех — занял золото; пришёл раньше всех — навсегда останется в серебре.
        </p>
      )}
...
function annotateDonors(donors) {
  ...
  donors.forEach((d, i) => {
    if (!d.addedAt) return;
    const t = new Date(d.addedAt).getTime();
    if (!Number.isNaN(t) && t < firstDate) {
      firstDate = t;
      firstIdx = i;
    }
  });
  ...
  return donors.map((d, i) => {
    ...
    if (i === firstIdx) roles.push('first-ever');
    return { ...d, roles };
  });
}

function HonorCard({ donor }) {
  const handle = donor.handle.startsWith('@') ? donor.handle : `@${donor.handle}`;
  const tgUrl = `https://t.me/${handle.replace(/^@/, '')}`;
  ...
  const palette = gold
    ? {
        ...
        kanji: '一',
        kanjiColor: 'text-amber-400/10 group-hover:text-amber-400/15',
        iconWrap: 'border-amber-500/40 bg-amber-500/10 text-amber-300',
        nameHover: 'group-hover:text-amber-200',
      }
    : {
        border: 'border-zinc-400/25 hover:border-zinc-300/50',
        bg: 'from-zinc-300/5 via-zinc-900/60 to-zinc-900/40',
        kanji: '初',
        kanjiColor: 'text-zinc-300/10 group-hover:text-zinc-200/15',
        iconWrap: 'border-zinc-400/40 bg-zinc-300/10 text-zinc-200',
        nameHover: 'group-hover:text-zinc-50',
      };
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      title={donor.note || handle}
      className={`group relative block overflow-hidden rounded-2xl border bg-gradient-to-br p-5 md:p-6 transition-colors ${palette.border} ${palette.bg}`}
    >
      <span
        aria-hidden
        className={`pointer-events-none absolute -right-3 -top-6 text-[140px] md:text-[180px] leading-none select-none transition-colors ${palette.kanjiColor}`}
      ...
      <div className="relative flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex flex-wrap items-baseline gap-x-3 gap-y-1">
            <span
              className={`text-xl md:text-2xl font-semibold font-mono text-zinc-100 transition-colors ${palette.nameHover}`}
            >
              {handle}
            </span>
            {donor.amount && (
            ...
            <p className="mt-1.5 text-xs text-zinc-500 leading-relaxed">
              {isTop && isFirst
                ...
                : isTop
                  ...
                  : donor.note || 'первый, кто поддержал проект'}
            </p>
          )}
        </div>
      </div>
